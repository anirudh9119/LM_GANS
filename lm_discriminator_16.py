...
def build_GAN_model(tparams, options):
    ...
    ind_new_mask = tensor.switch(tensor.eq(output_mask, 0),ind_max, temp_x)

    f_get =  theano.function([x, x_mask, bern_dist],[logit_shp, logit, probs, ind_max, ind_new_mask])

    # input
    emb = tparams['Wemb'][ind_new_mask]
    emb = emb.reshape([n_timesteps, n_samples, options['dim_word']])
    emb_shifted = tensor.zeros_like(emb)
    emb_shifted = tensor.set_subtensor(emb_shifted[1:], emb[:-1])
    emb = emb_shifted
    ...
    return trng, use_noise, x, x_mask, opt_ret, cost, f_get, bern_dist
