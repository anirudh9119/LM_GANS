...
class Convolutional(Initializable):
    ...
    def apply(self, input_):
        ...
        output = self.conv2d_impl(
             ...
             subsample=self.step,
             border_mode=(0, self.filter_size[1] / 2),
...
class ConvolutionalSequence(Sequence, Initializable, Feedforward):
    ...
    def __init__(self, layers, num_channels, num_pieces=1, batch_size=None,
                 image_size=None, tied_biases=False, **kwargs):
        self.layers = layers
        self.image_size = image_size
        self.num_channels = num_channels
        self.batch_size = batch_size
    ...
    def _push_allocation_config(self):
        ...
        for layer in self.layers:
            ...
            if layer.image_size not in (None, (None, None)):
            #if layer.image_size is not None:
                output_shape = layer.get_dim('output')
                image_size = output_shape[1:]
            if (layer.name[:4] == 'conv' and
                layer.application_methods[1].brick.name[:7] == 'convmax'):
                num_channels = layer.num_output_channels // self.num_pieces
            else:
...
class BNSequence(Initializable, Feedforward):
    """A sequence of convolutional operations.
    Parameters
    ----------
    layers : list
    ...
    def __init__(self, layers, num_channels, batch_size=None, image_size=None,
        ...
        super(BNSequence, self).__init__(**kwargs)
        self.layers = layers
        self.image_size = image_size
        self.num_channels = num_channels
        self.batch_size = batch_size
    ...
    def apply(self, input_, input_mask=None):
        for brick in self.children:
            if brick.name[:2] == 'bn':
                input_ = brick.apply(input_, input_mask)
            else:
                input_ = brick.apply(input_)
        return input_
    ...
    def get_dim(self, name):
        if name == 'input_':
            return ((self.num_channels,) + self.image_size)
        if name == 'output':
            return self.layers[-3].get_dim
        return super(BNSequence, self).get_dim(name)

    def _push_allocation_config(self):
        num_channels = self.num_channels
        image_size = self.image_size
        for layer in self.layers:
            if layer.name[:4] in ['conv', 'pool']:
                if self.border_mode is not None:
                    layer.border_mode = self.border_mode
                layer.tied_biases = self.tied_biases
                layer.image_size = image_size
